...
::: {.callout-tip title="some terminology" collapse="true"}
...
- Each track can have a `track_pool` of samples that the sequencer will rotate through. You can adjust *how* with the `play_order` parameter[^fn:playorder] in the **PARAMS** menu.
...
[^fn:playorder]: See the Sample Sequence section below for more on this.
...
![sample config](movements/sample_config_1.svg)
...
![sample sequence](movements/sample_seq.svg)
...
- You can adjust *how* the transport cycles through the pool with the `play_order` parameter in the **PARAMS** menu.
  - The options here are "forward", "backward", "random", or "fixed".
  - With "forward" and "backward", the first active step of a pattern will always be the first or last sample loaded in a pool, respectively.
  - With "fixed", the sample pool cycles across grid pads in the track's row, and remains locked. So, for a track pool containing 3 samples, the first five pads in the track's row would *remain* `pool_1 | pool_2 | pool_3 | pool_1 | pool_2`, and so on.
...
![sample levels](movements/sample_levels.svg)
...
![sample time](movements/sample_time.svg)
...
![tape config](movements/tape_config.svg)
...
![tape seq](movements/tape_seq.svg)
...
![track pool 1](movements/screen_1.png){width=50%}
...
![track pool 2](movements/screen_2.png){width=50%}
...
![track params](movements/screen_3.png){width=50%}
...
![sample envelope](movements/screen_6.png){width=50%}
...
![track pool 1](movements/screen_7.png){width=50%}
...
![track pool 2](movements/screen_8.png){width=50%}
...
![track params](movements/screen_9.png){width=50%}
...
![track waveform](movements/screen_10.png){width=50%}
...
![unidirectionality](movements/squelch_1.svg){width=50%}
...
![bidirectionality](movements/squelch_2.svg){width=50%}
...
![amplitude](movements/param_amp.svg)
...
![panning](movements/param_pan.svg)
...
![filtering](movements/param_filter.svg)
...
![scale](movements/param_scale.svg)
...
![interval](movements/param_interval.svg)
...
::: {.callout-note}
The following assumes you have a Mac or Linux, or you have Python installed on your computer.
:::
